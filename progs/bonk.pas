...
{         Source code (c)1992-93 S│LVεΓWΩRε            }
{ ──────────────────────────────────────────────────── }
Program TCoBEM;
...
Procedure TMainView.GetContents;
 ...
 begin
  ...
  pnum := 0;
  readchar
  IssueName :=  BNKFile, IssueName, 'A' );
 { <GET ISSUE NUMBER>
    <LOP TO GET ARTICLE NAMES, INSERT INTO CONTENTS & INDEX>
    while not eof( PageFile ) do
     begin
      inc( pnum );                    <- inc pagenumber
      BlockRead( PageFile, temp, 1 ); <- needs to be changed
      if Temp.Name <> '' then         <- ditto (should read for HS-B
       begin
         new(pc);
         pc^.Name := Temp.Name + ' -=> ' + N2S( pnum ); <-change the temp
         pc^.Num := pnum;
         TableOfContents^.Insert( pc );         the rest is great,except
         TheIndex^.Insert( pc );            I need to add a computer"page#"
       end;                                for the offset from the beginning
                                         so I can tell where to start reading
                                        my data for each page from.
  }
  NumPages := PNum;
  Dispose( Wait, Done );
 end;
...
Procedure TMainView.ChangeToPage( p : word );
 ...
 (*
  THiS should run as follows :
    <consult tableofcontents.offset for page p>
    <seek that offset>
    <read the page into memory / translate all the graphics stuff and such>
    <update issueview>
...
Procedure TMainView.GetFileName;
 var
  ...
  S : TSTREAM;
 begin
  Feeb := New( PFileDialog, Init( '*.BNK', 'Load',
          'B~o~NK~!~ issue to Load:', fdOpenButton + fdHelpButton, $00 ) );
  Feeb^.Flags := $0000;
  if DeskTop^.ExecView( Feeb ) <> cmCancel then
   begin
    Feeb^.GetFileName( BonkFileName );
    LoadBNK;
   end;
  dispose( Feeb, Done );
 end;
...
Procedure TMainView.OpenBNK;
 begin
  System.Assign( BonkFile, BonkFileName );
  System.Reset( BonkFile );
 end;

Procedure TMainView.CloseBNK;
 begin
  {$I-}
  System.Close( BonkFile );
  {$I+}
 end;
 ...
 begin
  ...
  with Inf^ do
   begin
    ...
    R.Assign( 7, 5, 35, 6 );
    Insert( New( PStaticText, Init( R, '(c)1993 S┼εΓL│NG S│LVεΓWΩΓε')));
    R.Assign( 15, 7, 25, 9 );
 ...
 begin
  ...
  MenuBar := New( PMenuBar, Init( R, NewMenu(
   NewSubMenu( '~≡~', hcNoContext, NewMenu(
    ...
    nil )))),
   NewSubMenu( '~B~o~NK~!', hcNoContext, NewMenu(
