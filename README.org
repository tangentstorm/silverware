...
| path  | contents                                                       |
|-------+----------------------------------------------------------------|
| [[./tree/master/apps][apps]] | refactored/modernized apps that compile and run under fpc      |
| [[./tree/master/old][old]] | the orignal code ( w/duplicates from multiple partial backups) |
| [[./tree/master/old_u][old_u]] | the original units ( most recent 'old' version of each )       |
| [[./tree/master/work][work]]  | broken / untried / unimproved programs from the 90's           |
| [[./tree/master/other][other]] | data files, images, fonts, junk.                               |


* about the units ( they've moved )

The [[./tree/master/old_u/][old_u]] directory contains a bunch of units, mostly dealing with textual user interfaces. These predate Borland's Turbo Vision and ( having been written by a self-taught teenager who knew nothing about event-driven user interfaces ), they offer an interesting viewpoint on how a "procedural" user interface library could work. ( The control flow is somewhat similar in design to that used in the Oberon operating system by pascal creator Niklaus Wirth, though of course not nearly as well-implemented, and also entirely based on 16-color console mode semi-graphics. )
...
- [[./tree/master/work/doth_a.pas][DOTH]] :: a top down adventure game, similar to the Kingdom of Kroz games ( there are several unreconciled versions of doth in the directory )
- [[./tree/master/work/dmm.pas][DMM]] :: the doth map-maker ( a level editor / tester )
- [[./tree/master/work/zmen.pas][ZMENU]] :: a full-screen menu program / application launcher
- [[./tree/master/work/xmen.pas][XMENU]] :: another menu program ( not sure what it does yet )
- [[./tree/master/work/ymen.pas][YMENU]] :: a primitive command line shell that also incorporates a menu
- [[./tree/master/work/life.pas][life]] :: an implementation of Conway's Game of Life ( cellular automata )
- [[./tree/master/old_u/adlstuff.pas][ADL]] :: the "advanced directory lister" - a colorful replacement for the =dir= command. ( that's =ls=, for all you young linux fans )
